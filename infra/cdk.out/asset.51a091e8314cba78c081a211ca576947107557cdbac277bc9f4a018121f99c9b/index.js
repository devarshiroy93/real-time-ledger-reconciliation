...
var handler = async (event) => {
  ...
  for (const record of event.Records) {
    ...
    const customer = await getLedgerEntry("ReconStack-CustomerLedger", txId);
    const processor = await getLedgerEntry("ReconStack-ProcessorLedger", txId);
    const core = await getLedgerEntry("ReconStack-CoreLedger", txId);
...
async function getLedgerEntry(table, txId) {
  const res = await ddb.send(new import_client_dynamodb.GetItemCommand({
    TableName: table,
    Key: { txId: { S: txId } }
  }));
